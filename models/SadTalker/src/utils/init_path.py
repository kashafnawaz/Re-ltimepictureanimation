...
def init_path(checkpoint_dir, config_dir, size=512, old_version=False, preprocess='crop'):
    ...
    if old_version:
        ...
        sadtalker_paths = {
                ...
                'audio2pose_checkpoint' : os.path.join(checkpoint_dir, 'audio2pose_00140-model.pth'),
                'audio2exp_checkpoint' : os.path.join(checkpoint_dir, 'audio2exp_00300-model.pth'),
    ...
    sadtalker_paths['use_safetensor'] =  use_safetensor # os.path.join(config_dir, 'audio2exp.yaml')
